...
from utils import set_seed, load_data, collate_fn, load_config
...
def main():
    ...
    set_seed(42)
    config, config_dict = load_config("./config/Bert_Config_exp1.json")
    ...
    train_loader = DataLoader(
        NewsDataset(train_texts, train_labels),
        batch_size=config.batch_size,
        shuffle=True,
        collate_fn=lambda batch: collate_fn(batch, tokenizer, config)
    )
    dev_loader = DataLoader(
        NewsDataset(dev_texts, dev_labels),
        batch_size=config.batch_size,
        shuffle=False,
        collate_fn=lambda batch: collate_fn(batch, tokenizer, config)
    )
    test_loader = DataLoader(
        NewsDataset(test_texts, test_labels),
        batch_size=config.batch_size,
        shuffle=False,
        collate_fn=lambda batch: collate_fn(batch, tokenizer, config)
    )
